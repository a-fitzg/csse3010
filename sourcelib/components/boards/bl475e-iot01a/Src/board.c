...
void BRD_LEDInit() {
  ...
  __BRD_GREEN1_LED_GPIO_CLK();
  __BRD_GREEN2_LED_GPIO_CLK();
  __BRD_YELLOW_LED_GPIO_CLK();
  __BRD_BLUE_LED_GPIO_CLK();
  

  /* Configure the GPIO_LED pin */
  GPIO_InitStructure.Pin = BRD_GREEN1_LED_PIN;
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStructure.Pull = GPIO_PULLUP;
  GPIO_InitStructure.Speed = GPIO_SPEED_FAST;
  HAL_GPIO_Init(BRD_GREEN1_LED_GPIO_PORT, &GPIO_InitStructure);

  GPIO_InitStructure.Pin = BRD_GREEN2_LED_PIN;
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStructure.Pull = GPIO_PULLUP;
  GPIO_InitStructure.Speed = GPIO_SPEED_FAST;
  HAL_GPIO_Init(BRD_GREEN2_LED_GPIO_PORT, &GPIO_InitStructure);

  GPIO_InitStructure.Pin = BRD_YELLOW_LED_PIN;
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStructure.Pull = GPIO_PULLUP;
  GPIO_InitStructure.Speed = GPIO_SPEED_FAST;
  HAL_GPIO_Init(BRD_YELLOW_LED_GPIO_PORT, &GPIO_InitStructure);
...
void BRD_LEDOn(uint8_t ledmask) {
	...
	if ((ledmask & BRD_GREEN1_LEDMASK) != 0) {
		BRD_GREEN1_LED_GPIO_PORT->BSRR |= BRD_GREEN1_LED_PIN;
	}

    if ((ledmask & BRD_GREEN2_LEDMASK) != 0) {
		BRD_GREEN2_LED_GPIO_PORT->BSRR |= BRD_GREEN2_LED_PIN;
	}

    if ((ledmask & BRD_YELLOW_LEDMASK) != 0) {
		BRD_YELLOW_LED_GPIO_PORT->BSRR |= BRD_YELLOW_LED_PIN;
	}
...
void BRD_LEDOff(uint8_t ledmask) {
    ...
    if ((ledmask & BRD_GREEN1_LEDMASK) != 0) {
  		BRD_GREEN1_LED_GPIO_PORT->BSRR |= BRD_GREEN1_LED_PIN << 16;
  	}

    if ((ledmask & BRD_GREEN2_LEDMASK) != 0) {
  		BRD_GREEN2_LED_GPIO_PORT->BSRR |= BRD_GREEN2_LED_PIN << 16;
  	}

    if ((ledmask & BRD_YELLOW_LEDMASK) != 0) {
  		BRD_YELLOW_LED_GPIO_PORT->BSRR |= BRD_YELLOW_LED_PIN << 16;
  	}
...
void BRD_LEDToggle(uint8_t ledmask) {
  //BRD_LED_GPIO_PORT->ODR ^= BRD_LED_PIN;
    ...
    if ((ledmask & BRD_GREEN1_LEDMASK) != 0) {
	    BRD_GREEN1_LED_GPIO_PORT->ODR ^= BRD_GREEN1_LED_PIN;
    }

    if ((ledmask & BRD_GREEN2_LEDMASK) != 0) {
	    BRD_GREEN2_LED_GPIO_PORT->ODR ^= BRD_GREEN2_LED_PIN;
    }

    if ((ledmask & BRD_YELLOW_LEDMASK) != 0) {
	    BRD_YELLOW_LED_GPIO_PORT->ODR ^= BRD_YELLOW_LED_PIN;
    }
...
void BRD_debuguart_init()
{
	...
	UART_debug.Instance = BRD_DEBUG_UART;
	...
	__BRD_DEBUG_UART_RX_GPIO_CLK();
  	...
  	GPIO_serialtx.Speed = GPIO_SPEED_FREQ_HIGH;
...
void BRD_debuguart_putc(unsigned char c)
{
    ...
    //UART_debug.Instance->TDR  = (uint8_t) c;
...
void BRD_debuguart_puts(unsigned char *c)
{
	...
	for (i = 0; i < (int) strlen(c); i++) {
        UART_debug.Instance->TDR  = (uint8_t) (*(c + i));
